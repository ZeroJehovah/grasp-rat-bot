...
2026-07-07 live 验证确认当前游戏 direct leave 为 `GET https://grasp-rat-game.h-e.top/leave?user_id=${userId}&token=${sessionToken}`，无请求体。Node 只带 session token 可能被 Cloudflare 返回 403；watchdog 现在把这类响应标记为 `cloudflareBlocked`，跳过普通裸重试，先执行已验证的 Clash rescue，再用同一 descriptor 进行 `after-clash` direct leave 重试。watchdog 不需要也不配置浏览器 HttpOnly clearance Cookie。
...
- 如果要处理 Cloudflare 403，`clash.validation.ok=true`
