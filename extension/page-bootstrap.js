(function () {
  ...
  const BOOTSTRAP_VERSION = '0.1.71';
  ...
  function schedulePanelInteractionFlush() {
    if (state.panelInteractionTimer) {
      ...
      timers.delete(state.panelInteractionTimer);
    ...
    timers.add(state.panelInteractionTimer);
  }
  ...
  async function loadStoredValues() {
    ...
    cfg = {
      ...
      loginStartEvidenceMs: Math.max(0, Number(readStored('loginStartEvidenceMs', DEFAULTS.loginStartEvidenceMs)) || DEFAULTS.loginStartEvidenceMs),
  ...
  async function maybeStartGameLogin(reason = 'watchdog', options = {}) {
    ...
    const rawStartLinuxDoLogin = force && typeof window.__graspRatBotRawStartLinuxDoLogin === 'function'
      ? window.__graspRatBotRawStartLinuxDoLogin
      : null;
    const startLoginFn = rawStartLinuxDoLogin || (typeof window.startLinuxDoLogin === 'function' ? window.startLinuxDoLogin : null);
      ...
      if (!loginMethods.length) {
        ...
        for (const item of loginMethods) {
          ...
          if (item.type === 'control') {
            ...
            methodResult = startLoginFn.call(window);
