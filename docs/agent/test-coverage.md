...
- Recent important coverage:
  ...
  - daily-summary self-test covers the `实际战斗收益统计` section: confirmed picked kills, chat-confirmed unpicked kills as `收益+0(未拾取)`, and HP-disadvantage exits as failed/zero-profit battle rows while excluding enemy-flee/target-switch outcomes;
