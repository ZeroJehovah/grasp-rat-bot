...
- bootstrap panels expose red/blue dashed-circle manual view-radius buttons for 151m and 502m plus the icon-only hunt-panel toggle on the current-time row, hide benign reconnect history after WebSocket recovery while preserving reconnect-churn display, and suppress stale relogin chrome after either a fresh alive self or a recent accepted login transition appears without treating stale `lastSelf` HP/Drop/stamina as fresh self evidence;
