...
- post-login zoom keeps the 500m target radius, uses the native/page `view r` as the stop condition, only sends zoom-out steps, disables blind fallback clicks by default, and keeps stable no-token session keys;
