...
- Objective build verification count: `40`.
...
`src/node/run-self-test.js` includes browserless runner coverage for shared `GRZ1` frame parsing, session/callback parsing, verified leave retries, direct WebSocket transport, frame statistics, state-store realtime/snapshot authority separation, local log retention/summary generation, legacy/demo state import and runner config hydration from persisted session/login-point state, canary evidence audit fixtures including deterministic run-id generation, selected-run id/window filtering, same-window cross-run contamination rejection, bootstrap-only final rejection, read-only action-leak rejection, combat-dry-run target evidence rejection, combat-live missing-action rejection, no-shoot action-log leakage, and combat-live missing shoot acknowledgement from action-log evidence, deployment evidence audit fixtures including safe/live/any env modes, profile/control mismatch rejection, blank live login-point rejection, and state-backed live session/login-point readiness, aggregate acceptance report fixtures including human evidence summaries and combat target counts, runner config/canary-profile parsing, non-`--once` loop-plan classification for recoverable exits versus explicit-stop/no-self/auth stops, status server redaction, deployment file anchors including runtime directory creation, read-only canary flow including normal deadline completion before post-run safety checks, dry-run decisions, safety exits, movement-only control, non-combat profit fallback gating, profit-live safety exits before profit movement under realtime Active/firing threat, formal profit-live snapshot-only system-coin fallback disablement, fresh self-kill snapshot player-drop pickup, fresh snapshot reward metadata enrichment for same-user realtime AFK targets, tighter browserless coin movement dead-zone behavior, combat dry-run target/aim/fire summaries, profit-live active combat when `combatEnabled=true`, profit-live suppression of Passive/non-firing combat action takeover, and guarded combat live behavior.

The guarded combat live tests verify that `controlMode=combat-live` remains command-suppressed unless `combatEnabled=true`, that enabled live combat sends combat velocity plus paced `shoot targetX targetY startX startY` through the action adapter, and that `shoot_ok` acknowledgements are retained in action state. These remain synthetic/fake-transport tests, but supervised VPS validation has also accepted `combat-dry-run-20260708T171555484Z` and `combat-live-20260708T173224614Z`; the live run sent movement commands and did not send shoot commands because fire gates never opened.

## External Watchdog Coverage

`node combat-log-service/server.js --self-test` now includes watchdog coverage for:
...
- active-rescue ordering where direct leave success does not switch Clash, Cloudflare-blocked direct leave switches validated Clash before the `after-clash` retry, and duplicate suppression prevents the same stale heartbeat window from sending repeated direct leave requests;
...
Live direct-leave proof was completed on 2026-07-07. `/watchdog/test-leave` and controlled active rescue returned 200 game JSON `event:"left"` using `GET /leave?user_id=${userId}&token=${sessionToken}` with browser request context. Token-only Node requests can fail with Cloudflare 403; watchdog self-test now covers the replacement path where a Cloudflare challenge result triggers validated Clash rescue and a single `after-clash` direct-leave retry, while invalid Clash validation prevents the retry.
...
- bootstrap panels expose red/blue dashed-circle manual view-radius buttons for 151m and 502m plus the icon-only hunt-panel toggle on the current-time row, hide benign reconnect history after WebSocket recovery while preserving reconnect-churn display, and suppress stale relogin chrome after either a fresh alive self, a recent accepted login transition, or current-page login confirmation after refresh/OAuth return/reopen appears without treating stale `lastSelf` HP/Drop/stamina as fresh self evidence; confirmed login suppression is cancelled only by a newer exit-detail event, not by login-point safety sampling alone;
