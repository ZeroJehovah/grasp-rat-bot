...
`src/node/run-self-test.js` includes browserless runner coverage for shared `GRZ1` frame parsing, session/callback parsing, verified leave retries, direct WebSocket transport, frame statistics, state-store realtime/snapshot authority separation, local log retention/summary generation, canary evidence audit fixtures including deterministic run-id generation, selected-run id/window filtering, same-window cross-run contamination rejection, read-only action-leak rejection, combat-live missing-action rejection, no-shoot action-log leakage, and combat-live missing shoot acknowledgement from action-log evidence, deployment evidence audit fixtures including safe/live/any env modes, profile/control mismatch rejection, and blank live login-point rejection, aggregate acceptance report fixtures including human evidence summaries, runner config/canary-profile parsing, status server redaction, deployment file anchors including runtime directory creation, read-only canary flow, dry-run decisions, safety exits, movement-only control, non-combat profit fallback gating, combat dry-run target/aim/fire summaries, and guarded combat live behavior.
