...
- `node scripts/verify-objective-build.js` now includes source/dist checks that movement and shooting prefer direct sends over the existing native page WebSocket, that movement repeats use the configured 50ms/220ms cadence, that nonzero direct movement does not sync local page keys/prediction, and that stop cleanup cancels/repeats direct control safely.
