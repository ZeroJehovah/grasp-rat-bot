...
- Daily summaries include a third `实际战斗收益统计` section in addition to login and active-combat tables. It lists only confirmed kills and battle failures/HP-disadvantage exits, excludes enemy-flee/target-switch/safety-avoidance rows, and reports actual picked reward separately from unpicked confirmed Drop or failed unearned Drop.
