...
- Daily active-combat result text should read as event descriptions rather than internal record states. Current labels include `主动退出本局`, `战后恢复`, `敌方逃离`, `切换交战目标`, and `交火停止`; `post-combat-timeout` with a `combat-target-retreating` start/last reason is rendered as `敌方逃离`. Safety movement that avoids invulnerable targets is not combat and is filtered out of the active-combat table, including legacy `recovery-avoid-humans` closures.
