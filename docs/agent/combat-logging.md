...
- Daily active-combat result text should read as combat outcomes rather than internal record states. Current visible labels include `胜利`, `失败`, `主动退出本局`, `战斗劣势主动退出`, `敌方逃离`, and `切换交战目标`; post-combat recovery waits and `post-combat-timeout` target loss are folded into `敌方逃离`, including targets that leave, teleport, disappear, or move out of range. Safety movement that avoids invulnerable targets is not combat and is filtered out of the active-combat table, including legacy `recovery-avoid-humans` closures.
