...
- Daily active-combat result text uses explicit state prefixes: `主动退出本局` for leaving the current game/session, `未退出本局` for in-game retreat/recovery/avoidance, and `战斗记录收口` for target switch or observation timeout that only closes that combat record.
