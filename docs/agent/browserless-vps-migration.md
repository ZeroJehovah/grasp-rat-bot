...
1. Monitor the active `profit-live` formal run `profit-live-20260708T174024319Z` through `systemctl`, `runner.jsonl`, `decisions.jsonl`, and the token-gated status API. The current live window is bounded by `GRASP_RAT_BROWSERLESS_READONLY_PROBE_MS=21600000`; the runner will still call verified `leave` when that window completes or a safety event fires.
