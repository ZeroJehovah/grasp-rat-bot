...
1. Deploy the snapshot Active profit/safety veto to the VPS, keep `GRASP_RAT_BROWSERLESS_CONTROL_MODE=profit-live` and `GRASP_RAT_BROWSERLESS_COMBAT_ENABLED=true`, rerun live deployment audit, and restart the service only after snapshot safety passes.
2. Monitor the new formal run through `systemctl`, `runner.jsonl`, `decisions.jsonl`, fresh `/snapshot`, and the token-gated status API. Confirm self HP safety, `shoot_ok` evidence when live combat opens, realtime combat authority, no Passive/non-firing combat action takeover in `profit-live`, no fresh snapshot Active target being attacked as AFK profit, `snapshot-player-drop` coin actions after self kills, and actual pickup by self Drop/backend balance records instead of kill-message estimates.
