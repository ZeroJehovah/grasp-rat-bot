...
1. Deploy the non-`--once` loop fix to the VPS, keep `GRASP_RAT_BROWSERLESS_CONTROL_MODE=profit-live` and `GRASP_RAT_BROWSERLESS_COMBAT_ENABLED=true`, rerun live deployment audit, and restart the service only after snapshot safety passes.
2. Monitor the new formal run through `systemctl`, `runner.jsonl`, `decisions.jsonl`, fresh `/snapshot`, and the token-gated status API. Confirm `runner-loop-wait` appears after recoverable safety exits and that a new `profit-live-*` run starts after the loop delay.
