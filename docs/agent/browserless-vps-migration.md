...
7. After all staged canaries have accepted evidence, run `sudo node scripts/browserless-acceptance-report.js --log-dir /var/log/grasp-rat-browserless --day YYYY-MM-DD --fail-on-incomplete` and keep its output with the cutover handoff.
...
10. Mark `headless-demo/` superseded only after the production runner canaries and aggregate acceptance report above have accepted evidence; until then it remains a diagnostic protocol probe.
