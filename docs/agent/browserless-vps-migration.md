...
1. After the `profit-live` safety/dead-zone/fallback/AFK-metadata fix is validated and pushed, pull it on the VPS, rerun live env audit, restart `grasp-rat-browserless-runner`, and monitor the new formal run through `systemctl`, `runner.jsonl`, `decisions.jsonl`, and the token-gated status API. The live window remains bounded by `GRASP_RAT_BROWSERLESS_READONLY_PROBE_MS=21600000`; the runner will still call verified `leave` when that window completes or a safety event fires.
