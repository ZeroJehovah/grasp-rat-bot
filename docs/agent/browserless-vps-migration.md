...
The first phase was a small probe under `headless-demo/`. That probe is now superseded for production operation and remains only a diagnostic protocol tool. The production browserless runner runs as the `systemd` service `grasp-rat-browserless-runner`.
...
- 2026-07-08: Direct pre-login `/snapshot?user_id=<id>&token=<token>` fetch from VPS worked before WS join. A later cache-busted snapshot safety probe with a persisted login point validated this path.
...
- 2026-07-09: VPS movement-only revalidation after the velocity-command quantization fix could not start because the state-backed direct WS join returned HTTP 403 before any frames. A follow-up auth check showed direct `/snapshot` returns HTTP 200 with the same public snapshot shape for real, empty, and wrong tokens, so snapshot success is not session-validity evidence. Treat direct WS open with real `userId`/token as the session-validity gate. `src/node/browserless/ws-transport.js` now surfaces Node `ws` unexpected-response status/content-type/body snippets so future 403s are easier to diagnose. The current production state needs a fresh manual authorization before canary validation can continue.
- 2026-07-09 Asia / 2026-07-08 UTC: The production session was refreshed through the token-gated status callback API without printing the token. Direct WS open with the persisted user id/token succeeded, and `leaveWithVerification` confirmed `joined: UserRecordOnly` plus `current_join_mode: None`.
- 2026-07-08 UTC: Movement-only revalidation after velocity quantization accepted with run id `movement-only-20260708T170232586Z`, window `2026-07-08T17:02:32.586Z .. 2026-07-08T17:04:34.365Z`, 2401 decoded frames, 117 decisions, 118 movement-command entries, 0 shoot commands, verified `leave`, and self movement from about `(5999,66268)` to `(-1130,69008)`.
- 2026-07-08 UTC: Non-combat profit revalidation after velocity quantization accepted with run id `non-combat-profit-20260708T170714579Z`, window `2026-07-08T17:07:14.579Z .. 2026-07-08T17:09:16.798Z`, 2409 decoded frames, 117 decisions, 118 movement-command entries, 0 shoot commands, verified `leave`, and the same movement-scale proof.
- 2026-07-08 UTC: Combat dry-run accepted with run id `combat-dry-run-20260708T171555484Z`, window `2026-07-08T17:15:55.484Z .. 2026-07-08T17:17:57.188Z`, 2406 decoded frames, 117 combat rows, 8 realtime target rows, 0 movement commands, 0 shoot commands, and verified `leave`.
- 2026-07-08 UTC: Combat live accepted with run id `combat-live-20260708T173224614Z`, window `2026-07-08T17:32:24.614Z .. 2026-07-08T17:34:26.750Z`, 2408 decoded frames, 117 combat rows, 115 realtime target rows, 118 movement-command entries, 0 shoot commands because fire gates never opened, and verified `leave`.
- 2026-07-08 UTC: Final aggregate cutover readiness passed with `sudo node scripts/browserless-acceptance-report.js --log-dir /var/log/grasp-rat-browserless --day 2026-07-08 --fail-on-incomplete`. It selected read-only `read-only-20260708T142429656Z`, forced-stop `read-only-20260708T145958570Z`, movement-only `movement-only-20260708T170232586Z`, profit `non-combat-profit-20260708T170714579Z`, combat-dry-run `combat-dry-run-20260708T171555484Z`, and combat-live `combat-live-20260708T173224614Z`.
- 2026-07-08 UTC: Formal production play was started through systemd with `GRASP_RAT_BROWSERLESS_DRY_RUN=false`, blank `GRASP_RAT_BROWSERLESS_CANARY_PROFILE`, `GRASP_RAT_BROWSERLESS_CONTROL_MODE=profit-live`, `GRASP_RAT_BROWSERLESS_READ_ONLY=false`, `GRASP_RAT_BROWSERLESS_COMBAT_ENABLED=false`, and `GRASP_RAT_BROWSERLESS_READONLY_PROBE_MS=21600000`. Live env audit passed before restart, and run id `profit-live-20260708T174024319Z` opened WS at `2026-07-08T17:40:25.766Z`; after about 75s it had 74 decisions, 74 movement commands, no runner error, and the service remained active.
- 2026-07-08 UTC: The first formal `profit-live` run later failed at `2026-07-08T18:51:01.390Z` with `canary-failed` / `no-self`. Postmortem logs showed a realtime firing Active target close enough to damage self from 100 HP to 14 HP, while `profit-live` still selected profit actions because combat was diagnostic-only when `combatEnabled=false`. The same window repeatedly targeted a snapshot-only 1-coin drop about 761cm away; the generic 900cm movement dead zone treated the coin as reached, so the runner stopped without pickup income.
- 2026-07-09: Browserless `profit-live` safety handling now converts realtime Active/firing combat dry-run targets into a safety exit before profit movement. It leaves on a threatening target inside normal attack range, or when injured with a threatening target inside engage range, and canary safety evaluation runs before the action adapter sends any profit movement. Browserless coin targets now use a 150cm movement dead zone by default instead of the generic 900cm dead zone.
...
- 2026-07-09 UTC: The first run after the self-kill player-drop pickup fix showed `snapshot-player-drop` actions and snapshot self `death_drop_coins` increasing from 161 to 162, confirming actual pickup evidence instead of kill-message estimates. The same monitoring found `profit-live` with `combatEnabled=true` selecting combat-live for a moving, non-firing Passive target ahead of a better AFK profit target. `profit-live` now only promotes realtime Active/firing targets into live combat actions; Passive/non-firing combat candidates remain diagnostic-only in that mode, while explicit `combat-live` mode is unchanged.
- 2026-07-09 UTC: After deploying the `profit-live` Passive/non-firing combat-action gate, run `profit-live-20260709T012930981Z` produced verified pickup evidence before failing: previous final self `death_drop_coins` was 162 and a fresh snapshot at `2026-07-09T01:39:03Z` showed 266, so actual self Drop increased by 104; `stamina_1d_remaining_milli` moved from 18,735,521 to 18,489,920, a 245,601 unit 1d-budget spend. The run then failed with `no-self` after snapshot messages showed `MrHuang killed 文月` at tick 692965. Root cause: direct `pos` frames for user 14875 omitted `current_join_mode`, so browserless profit classified it as a Passive/AFK reward target, while the fresh same-user snapshot had `current_join_mode:"Active"` at a matching position. Browserless profit now treats fresh, position-matched snapshot Active mode as profit/safety veto metadata: it blocks AFK-profit selection and can trigger `profit-live-snapshot-active-threat` safety exit, but it still does not use snapshot mode for combat target, aim, or fire decisions.
- 2026-07-09 10:01 CST / 2026-07-09 02:01 UTC: VPS observation after `2bf0045` found the latest run `profit-live-20260709T015902559Z` exited safely on `profit-live-snapshot-active-threat` with verified `leave`, final HP 100, Drop 16, coins 1000, 1d stamina remaining 19,937,260, and death count still 2. The latest run spent 62,740 1d stamina since the previous verified leave and gained +0 Drop by self Drop evidence. It also exposed a runner lifecycle issue: non-`--once` service operation stayed systemd-active after the run ended but did not start another game cycle. The runner now loops recoverable exits after `GRASP_RAT_BROWSERLESS_LOOP_DELAY_MS`, while `explicit-stop`, `no-self`, direct leave failure, and auth-like 403 errors stop for inspection. Short current observation notes live in `docs/agent/browserless-vps-observation.md`.
- 2026-07-09 10:36 CST / 2026-07-09 02:36 UTC: `25c69db` was pulled to the VPS, `sudo node scripts/browserless-deployment-audit.js --env-mode live --fail-on-incomplete` passed, and `grasp-rat-browserless-runner` was restarted. Run `profit-live-20260709T022052916Z` is still active; fresh snapshot tick `760807` showed HP 100, Drop 30, 1d stamina remaining 19,542,915, death count 2, and 0 Active enemies within 170m. Against the previous verified leave baseline, this is Drop +14, 1d stamina spent 394,345, and death count +0.
- 2026-07-09: Backend pickup review found repeated 1-coin player-drop rows from `lopoincare`/`#19369`. VPS decision logs confirmed this was not only post-kill pickup: `profit-live-20260709T022052916Z` selected `#19369` as an AFK `attack` target with Drop 1 from `2026-07-09T02:32:53Z` through `02:33:23Z`. Root cause was a browserless-only default drift: the Node adapter used `attackMinAfkDrop=1`, while the browser runtime default is `attackMinAfkDrop=3`. Browserless AFK-profit selection now uses the same default threshold and has a regression test that rejects Drop 1 while keeping Drop 3 eligible.
- 2026-07-09 11:02 CST / 2026-07-09 03:02 UTC: `6142c2a` was pulled to the VPS after an explicit-stop verified leave from `profit-live-20260709T025717918Z` (HP 100, Drop 65, 1d stamina remaining 19,037,928, death count 2). VPS self-test, browserless runner self-test, live deployment audit, and active systemd audit passed. The restarted fixed run `profit-live-20260709T030155471Z` is active; sampled decisions from `03:02:24Z..03:02:44Z` selected Drop 12 as best and contained only Drop 12/5/4/3 AFK candidates, with no Drop 1/2 AFK candidates.
- 2026-07-09 11:10 CST / 2026-07-09 03:10 UTC: The fixed run `profit-live-20260709T030155471Z` exited safely on `profit-live-snapshot-active-threat` with verified `leave`, HP 100, Drop 67, 1d stamina remaining 18,939,948, and death count 2. Compared with the pre-fix-deploy baseline, this confirms Drop +2 by self Drop evidence with 97,980 1d stamina spent and no new death. Later retries are in `snapshot-safety-retry` because Active player `Caoyinsu/#30919` is 14,285cm from the learned login point.
- 2026-07-09 11:32 CST / 2026-07-09 03:32 UTC: The loop recovered after safety retries. Run `profit-live-20260709T032044340Z` opened WS, attacked Passive Drop 4/4/3 targets, pursued corresponding player-drop coins, then exited safely on `profit-live-snapshot-active-threat` at `03:24:57Z` with verified `leave`, HP 100, Drop 73, 1d stamina remaining 18,831,651, and death count 2. Compared with the previous verified fixed-code leave this is Drop +6 by self Drop evidence with 108,297 1d stamina spent and no new death. Decision audit from `03:20Z` through `03:31:52Z` found 0 Drop 1/2 AFK candidates/selections. Current run `profit-live-20260709T032729247Z` remains active/unverified, last seen seeking Passive Drop 3 target `#29433`.

## Next Plan

1. Monitor the new formal run through `systemctl`, `runner.jsonl`, `decisions.jsonl`, fresh `/snapshot`, and the token-gated status API. Confirm `runner-loop-wait` appears after recoverable safety exits and that a new `profit-live-*` run starts after the loop delay.
2. Keep `GRASP_RAT_BROWSERLESS_CONTROL_MODE=profit-live` and `GRASP_RAT_BROWSERLESS_COMBAT_ENABLED=true` for formal operation unless a safety issue requires reverting to a staged canary profile.
3. Confirm self HP safety, `shoot_ok` evidence when live combat opens, realtime combat authority, no Passive/non-firing combat action takeover in `profit-live`, no Drop 1 AFK targets entering attack decisions, no fresh snapshot Active target being attacked as AFK profit, `snapshot-player-drop` coin actions after self kills, and actual pickup by self Drop/backend balance records instead of kill-message estimates.
4. Keep `/snapshot` as pre-login safety and guarded profit evidence only. Direct WS open with the real user id/token remains the session-validity gate, and combat target/aim/fire must remain realtime-only.
5. Use `GRASP_RAT_BROWSERLESS_CANARY_PROFILE` for future staged revalidations. Use explicit `GRASP_RAT_BROWSERLESS_CONTROL_MODE=profit-live` only for formal profit operation, not for the `profit` canary acceptance audit.
...
7. Keep `headless-demo/` only as a diagnostic protocol probe; production operation is the `grasp-rat-browserless-runner` service.
