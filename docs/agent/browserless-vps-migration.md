...
- 2026-07-09 10:31 CST / 2026-07-09 02:31 UTC: `25c69db` was pulled to the VPS, `sudo node scripts/browserless-deployment-audit.js --env-mode live --fail-on-incomplete` passed, and `grasp-rat-browserless-runner` was restarted. New run `profit-live-20260709T022052916Z` opened and was active; fresh snapshot tick `755344` showed HP 100, Drop 28, 1d stamina remaining 19,659,167, death count 2, and 0 Active enemies within 170m. Against the previous verified leave baseline, this is Drop +12, 1d stamina spent 278,093, and death count +0.
...
3. Confirm self HP safety, `shoot_ok` evidence when live combat opens, realtime combat authority, no Passive/non-firing combat action takeover in `profit-live`, no fresh snapshot Active target being attacked as AFK profit, `snapshot-player-drop` coin actions after self kills, and actual pickup by self Drop/backend balance records instead of kill-message estimates.
