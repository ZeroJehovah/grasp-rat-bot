...
- Browserless runner defaults:
  ...
  - `GRASP_RAT_BROWSERLESS_LOOP_DELAY_MS = 30000`
  ...
  - The CLI accepts `--read-only`, `--movement-only`, `--non-combat-profit`, `--profit-live`, `--combat-dry-run`, `--combat-live`, `--combat-enabled`, `--no-combat-enabled`, `--canary-profile`, `--control-mode`, `--dry-run`, `--live`, `--once`, `--data-dir`, `--log-dir`, `--status-host`, `--status-port`, `--web-token`, `--user-id`, `--session-token`, `--read-only-probe-ms`, `--frame-gap-alert-ms`, `--snapshot-path`, `--decision-interval-ms`, `--stale-self-ms`, `--no-self-grace-ms`, `--stamina-exhausted-below-ms`, `--movement-command-interval-ms`, `--movement-target-dead-zone-cm`, `--movement-settlement-frames`, `--combat-shoot-min-interval-ms`, `--login-point-x`, `--login-point-y`, and `--login-point-hp`. Live active-player shooting requires `combatEnabled=true` with either `controlMode=combat-live` or `controlMode=profit-live`; `profit-live` may also shoot visible AFK targets, ignores ordinary snapshot-only system coin fallback, and allows snapshot player drops only when they match fresh self kill messages.
