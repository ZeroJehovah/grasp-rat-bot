...
- Combat:
  ...
  - active combat tracks recent self/target HP samples over a 6s exchange window. If self is taking enough damage and the predicted time-to-death is worse than predicted time-to-kill by the configured safety factor, combat exits via `combat-hp-disadvantage-leave` with `tradeEstimate` evidence.
