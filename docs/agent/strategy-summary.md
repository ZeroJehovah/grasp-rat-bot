...
- Combat:
  ...
  - combat aim is behavior-driven: it can switch to live/native/render precision when selected-target and current visible coordinates diverge, when real incoming bullet pressure makes current visible position more reliable than intercept lead, when server-position stall makes live movement more useful, or when target movement is mostly radial;
