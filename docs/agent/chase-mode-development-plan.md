...
追杀模式用于让用户手动标记一个或多个高 Drop 敌方玩家，并让脚本在不破坏现有生存、战斗、回血、退出逻辑的前提下，把这些目标当作高优先级收益目标处理。本文是开发计划，不包含实现代码。

## Goals

- 在脚本面板“当前行为”行右侧增加追杀面板开关。
- 在现有信息面板右侧显示/隐藏一个样式一致的新追杀面板。
- 追杀面板列出当前游戏中的高价值敌方玩家：`Drop >= 10`。
- 面板候选来自视野/native/render/realtime 数据和快照/minimap/global 状态的合并视图。
- 列表显示“Drop 最高的 10 个”和“距离最近的 10 个”的并集，按玩家去重，最多 20 个。
- 每个玩家展示名称、血量、Drop、距离、来源/状态提示，以及追杀/取消操作。
- 用户可同时标记多个追杀目标，脚本按距离近到远选择可追杀目标。
- 追杀行为优先级高于普通 `amount=1` 金币和挂机玩家击杀，但低于现有战斗、回血、退出与安全逻辑。
- 目标在游戏内但射程外时向其靠近；目标在射程内且 native/realtime/render 可见时进入现有战斗目标流程。
- 追杀状态跨页面刷新保留；敌方或己方下线不自动清除。
- 成功击杀、目标 Drop 降到 10 以下、目标进入白名单时自动清除追杀状态。
- 连续追杀时，追杀成功后必须优先拾取该目标爆出的金币。

## Terminology

- **Chase mode / 追杀模式**：本文新增功能的内部名称，代码建议使用 `chaseMode`。不要使用 `pursuit` 命名，避免和现有“被追击退出”逻辑混淆。
- **Chase target / 追杀目标**：用户手动标记为追杀中的敌方玩家。
...
- 不允许对白名单玩家发起追杀。面板按钮应显示说明文案，运行时如发现已持久化白名单目标，必须自动清除。
- 不使用 snapshot/minimap 数据进行 combat target、aim 或 fire 决策。远程数据只能驱动 `seek-enemy` 式接近动作。
- 不为某个具体玩家名或单次战斗窗口定制逻辑。
- 不把追杀做成纯 coin-per-stamina 分数压过生存行为；它只进入 profit band，并带更高的 profit 优先级。
...
- 在“当前行为”行使用 flex 行布局。
- 左侧显示现有 `当前行为：...` 文本。
- 右侧增加一个紧凑按钮，建议文案 `追杀`，可附带追杀中数量，如 `追杀 2`。
- 按钮样式沿用现有 dot/pill 控件：深色半透明背景、`rgba(148,163,184,.24)` 边框、24px 高度、mono 字体、8px 或 pill 圆角。
- 按钮状态：
  - 面板隐藏：普通灰蓝边框。
  - 面板显示：高亮边框和轻微 glow。
  - 存在追杀目标：使用 amber/red 强调并显示数量。

点击按钮只切换追杀面板显示状态，不直接改变追杀目标。
...
- 面板显示状态可放在 bootstrap A 的 localStorage，例如 `graspRatChasePanelVisible`，只影响 UI 展开/收起，不影响追杀目标状态。

### Chase Panel Content

面板结构建议：

- 顶部标题行：`追杀目标`，右侧显示候选数量和追杀中数量。
- 列表区最多 20 行，按以下策略排序：
  - 默认优先显示正在追杀中的目标。
  - 其他候选按 `追杀中 desc -> Drop desc -> distance asc -> name` 排序，或使用两个小分组“高 Drop / 最近”。
  - 需求要求“Drop最高10个”和“距离最近10个”的并集；排序可读性可以按上述规则处理，但候选集合必须来自这两个集合的 union。
- 每行字段：
  - 名称：优先 `name`，缺失时 `#user_id`。
  - HP：已知血量；未知显示 `HP ?`。
  - Drop：整数；Drop 未知或低于阈值的追杀目标仅在“追杀中但暂不可用”状态显示。
  - 距离：从当前 self 到目标坐标；未知显示 `距离 ?`。
  - 来源/状态提示：`视野`、`快照`、`minimap`、`离线/未刷新`、`白名单`、`体力不足`、`射程内` 等短文本。
  - 操作：
    - 非追杀、非白名单、`Drop >= 10`：`追杀`。
    - 已追杀：显示炫酷但克制的 `追杀中` 标签，并显示 `取消` 按钮。
    - 白名单：按钮替换为 `白名单` 或 `不可追杀`，禁用。
    - Drop 低于 10：如果不是已追杀候选，不显示；如果已追杀但刚发现 Drop 低于 10，运行时应清除，UI 下一帧消失或显示清除状态。

“追杀中”标签建议使用小型 pill：红/amber 渐变边框、微弱 glow、字重 800，不使用大面积动画，避免影响游戏视野和性能。
...
- `summarizeChaseModeStatus()`
  - 返回追杀目标、候选列表、当前选择、不可追杀原因、最近自动清理原因。
...
追杀目标状态应放在 browser runtime B 层持久化，不依赖 A 层 UI 是否展开：
...
- 按 `user_id` 去重；没有 id 的数据不允许作为可追杀目标，只能做诊断。
- 选择面板候选集合：Drop top 10 union nearest top 10。
- 选择当前追杀目标：已追杀目标中距离最近且可行动的目标。
- 计算追杀动作的 stamina 预算需求和不可行动原因。
...
- 追杀中目标即使暂时不可见也保留在 `targets`，但不进入可行动候选。
...
高价值可见金币、脚下金币、追杀之间存在边界：

- 需求明确追杀高于“普通金额=1金币”和“挂机玩家击杀”，但未要求高于高价值金币。
- 建议保守处理：`amount >= highValueCoinPriorityAmount` 的高价值可见金币仍保留现有优先级；`amount=1` 或普通安全金币低于追杀。
- 战斗后掉落金币必须高于追杀，保证“追杀后一定要拾取爆出的金币”。
...
- 不直接手写开火。
- 将追杀目标交给现有 `buildCombatAction()` / combat target selection 路径，或新增受控入口 `pickChaseCombatTarget()`，最终仍产出 `combat: true` action。
...
- 已经进入 combat 的追杀目标应保持到现有 combat disengage/stop-loss 清除，不因为另一个追杀目标更近而打断战斗。
...
追杀目标被忽略不代表清除追杀状态。体力恢复后可重新进入可行动候选。
...
- 在追杀 attack/combat action 上标记 `chaseMode.targetId/drop/name`。
...
- 对连续追杀，post-attack drop coin 和 wait 必须先于下一个 chase approach/combat 执行。

## Whitelist Plan

白名单必须在 UI 和 runtime 双层保护：

- 面板候选聚合时直接排除白名单；如果需要展示追杀中但已白名单的旧目标，显示 `白名单，已清除` 并触发清理。
- `setChaseTarget()` 二次检查 `isWhitelistedTarget()`，失败返回 `{ ok:false, reason:'target-whitelisted' }`。
- `chooseAction()` 每 tick 维护追杀目标时，如果目标当前名称命中白名单，自动清除。
- 面板按钮对已知白名单玩家显示 `白名单` / `不可追杀`，不可点击。
...
- `src/browser/runtime/profit-arbitration-runtime.js` 或相关 profit modules
  - 如追杀接入 opportunity/final arbitration，需要确保 focus key、score、reason 可诊断。
...
- `userscript/grasp-rat-bootstrap.user.js`
  - 当前行为行按钮、追杀面板 DOM、调用 bot API。
- `extension/page-bootstrap.js`
  - 同步当前行为行按钮、追杀面板 DOM、调用 bot API。
...
- 追杀中目标 Drop 明确低于 10 时产生 clear intent。
- 多追杀目标按距离选择，stick window 内不抖动。
...
如果追杀逻辑基于具体战斗记录调整战斗策略，还必须按 `AGENTS.md` 的 combat replay 要求跑对应离线 replay。首次实现若没有具体战斗记录，运行现有 self-tests 和 runtime tests 即可。
...
4. 实现 userscript/extension 面板按钮与追杀面板，支持 set/clear API。
...
- 高价值可见金币是否应继续高于追杀：本计划建议保留现有高价值金币优先级，只让追杀高于普通 `amount=1` 金币和 AFK 击杀。
- 远程目标的 snapshot/minimap 时效阈值：建议先沿用现有 freshness，若面板出现陈旧目标再调小。
- 追杀目标 Drop 低于 10 的清理是否要求 native/realtime 确认：本计划建议“明确同 id 新鲜数据”即可，native 优先，snapshot 可作为远处清理依据但需要 freshness。
- 追杀目标下线很久是否要过期：需求要求下线和刷新不清除，因此不设自动过期；只在 UI 标注 `未刷新`。
