...
- Node `fetch` is tried first. On fetch error, or Cloudflare-style HTTP 403/HTML challenge, the script retries the same URL through `curl` with the same browser-like headers, `--location`, `--max-time`, and `--retry 2`.
