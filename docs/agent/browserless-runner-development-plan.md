...
| Commit 14: Add Combat Dry-Run Mode | Complete | `src/node/browserless/combat-adapter.js` maps realtime `pos` self/entities/bullets into target selection, movement, aim, and fire-discipline dry-run summaries; `controlMode=combat-dry-run` logs local `combat` JSONL windows and suppresses movement/shoot commands. The 2026-07-08 UTC VPS dry-run canary `combat-dry-run-20260708T171555484Z` passed audit with 117 combat rows, 8 realtime target rows, 0 movement commands, 0 shoot commands, and verified `leave`. |
| Commit 15: Add Guarded Combat Live Mode | Complete | `controlMode=combat-live` plus explicit `combatEnabled=true` enables guarded realtime combat movement and paced `sendShoot()` commands through `src/node/browserless/action-adapter.js`; default config keeps live combat off, shoot commands require fire-discipline/range/reserve gates, `shoot_ok` acknowledgements are surfaced in action state, and synthetic self-tests cover disabled and enabled paths. The 2026-07-08 UTC VPS combat-live canary `combat-live-20260708T173224614Z` passed audit with 117 combat rows, 115 realtime target rows, 118 movement-command entries, 0 shoot commands because fire gates never opened, and verified `leave`. |
...
| Commit 17: Production Canary And Cutover Docs | Complete | Runner config supports `--canary-profile read-only|movement-only|profit|combat-dry-run|combat-live` and `GRASP_RAT_BROWSERLESS_CANARY_PROFILE` for staged rollout without code edits; `docs/agent/current-state.md`, `current-architecture.md`, `browserless-vps-migration.md`, `test-coverage.md`, and operator docs record the production canary/cutover surface while preserving the browser bot fallback. The production runner canaries and aggregate cutover report are accepted, so `headless-demo/` is superseded for production operation and remains only a diagnostic probe. |
| Commit 18: Add Canary Evidence Audit | Complete | `scripts/browserless-canary-audit.js` reads browserless JSONL logs for a UTC day and checks staged profile evidence for clean finish or forced-stop validation, verified leave, snapshot safety, decoded frames, decision logging, no forbidden shoot/action behavior, realtime combat authority, and combat dry-run suppression. `node grasp-rat-bot.js --self-test` covers normal read-only and forced-stop audit fixtures, and the VPS canaries were accepted through this deterministic audit path. |
...
| Commit 20: Add Aggregate Acceptance Report | Complete | `scripts/browserless-acceptance-report.js` aggregates deployment, read-only, forced-stop, movement-only, profit, combat-dry-run, and combat-live audit results into one cutover-readiness report. The canary audit now selects the latest clean finish for normal checks and explicit-stop evidence for forced-stop checks, so multiple same-day staged runs can be reviewed together. `node grasp-rat-bot.js --self-test` covers aggregate success and missing-profile failure fixtures, and the 2026-07-08 UTC VPS aggregate report passed with all staged run ids selected. |
...
| Commit 34: Quantize Browserless Velocity Commands | Complete | `src/node/browserless/ws-transport.js` now sends direct WS velocity commands using the same rounded/clamped `-1/0/1` format as the browser native transport instead of fractional vectors. Self-tests cover fractional and out-of-range velocity command formatting. The 2026-07-08 UTC movement-only revalidation `movement-only-20260708T170232586Z` and profit revalidation `non-combat-profit-20260708T170714579Z` both passed with 118 movement-command entries and clear self-position movement. |
| Commit 35: Add Explicit Profit-Live AFK Mode | Complete | `controlMode=profit-live` is now a separate explicit mode for ordinary coins plus visible AFK targets. The `profit` canary profile remains mapped to `non-combat-profit` and keeps its no-shoot audit contract; `profit-live` blocks AFK profit while a visible Active threat exists and does not replace combat-live. Self-tests cover AFK target selection, Active-threat blocking, and AFK shoot command emission. Formal production play is active on the VPS as `profit-live-20260708T174024319Z` with dry-run off, blank canary profile, combat disabled, and a 6 hour bounded window. |
| Commit 36: Surface WS Join Rejection Details | Complete | `src/node/browserless/ws-transport.js` now records Node `ws` `unexpected-response` status, content type, and a short body snippet in the connection error. The 2026-07-09 Asia reauthorization refreshed production state through the status callback API; direct WS open with the stored user id/token then succeeded, while `/snapshot` remains non-authoritative for auth validity. |
...
## External VPS Validation Completed

Local implementation work is complete through Commit 36, VPS systemd deployment validation passed on 2026-07-08, all staged production canaries have accepted evidence, and the aggregate cutover report passed on 2026-07-08 UTC. `headless-demo/` is superseded for production operation and remains only a diagnostic protocol probe.

Use the production service path and audit commands from `docs/agent/browserless-vps-migration.md` and `docs/agent/browserless-runner-operator.md` for future revalidations.
...
- 2026-07-08: A 10 minute combat-dry-run attempt `combat-dry-run-20260708T152147589Z` cleanly finished with verified `leave`, 11982 decoded/self-observed frames, 584 dry-run combat diagnostic rows, 0 movement commands, and 0 shoot commands, but it had 0 realtime combat targets/candidates. This is useful safety evidence but is not accepted Commit 14 canary evidence; Commit 33 tightened the audit so targetless combat diagnostic rows cannot satisfy combat canary acceptance.
- 2026-07-08: An active-search navigation attempt toward snapshot-located Active player `JeremyOp` sent 358 direct velocity commands over 182.8s and verified `leave`, but self remained at the starting coordinates. This exposed that browserless direct WS velocity commands were fractional while the browser native transport rounds/clamps `vel` payloads to `-1/0/1`; Commit 34 fixed the command format, and the later movement/profit revalidations below proved movement.
- 2026-07-09 Asia / 2026-07-08 UTC: The production session was refreshed through the status callback API, direct WS open succeeded with the stored user id/token, and verified `leave` confirmed `joined: UserRecordOnly` plus `current_join_mode: None`.
- 2026-07-08 UTC: Movement-only revalidation after Commit 34 passed with run id `movement-only-20260708T170232586Z` over `2026-07-08T17:02:32.586Z .. 2026-07-08T17:04:34.365Z`; audit accepted 2401 decoded frames, verified `leave`, 117 decisions, 118 movement commands, 0 shoot commands, and self-position movement from about `(5999,66268)` to `(-1130,69008)`.
- 2026-07-08 UTC: Non-combat profit revalidation after Commit 34 passed with run id `non-combat-profit-20260708T170714579Z` over `2026-07-08T17:07:14.579Z .. 2026-07-08T17:09:16.798Z`; audit accepted 2409 decoded frames, verified `leave`, 117 decisions, 118 movement commands, and 0 shoot commands.
- 2026-07-08 UTC: Combat dry-run passed with run id `combat-dry-run-20260708T171555484Z` over `2026-07-08T17:15:55.484Z .. 2026-07-08T17:17:57.188Z`; audit accepted 2406 decoded frames, 117 combat rows, 8 realtime target rows, 0 movement commands, 0 shoot commands, and verified `leave`.
- 2026-07-08 UTC: Combat live passed with run id `combat-live-20260708T173224614Z` over `2026-07-08T17:32:24.614Z .. 2026-07-08T17:34:26.750Z`; audit accepted 2408 decoded frames, 117 combat rows, 115 realtime target rows, 118 movement commands, 0 shoot commands because fire gates never opened, and verified `leave`.
- 2026-07-08 UTC: Final aggregate cutover readiness passed with `sudo node scripts/browserless-acceptance-report.js --log-dir /var/log/grasp-rat-browserless --day 2026-07-08 --fail-on-incomplete`, selecting the accepted read-only, forced-stop, post-fix movement/profit, combat-dry-run, and combat-live run ids.
- 2026-07-08 UTC: Formal production play started through systemd as run id `profit-live-20260708T174024319Z` with dry-run off, blank canary profile, `controlMode=profit-live`, read-only false, combat disabled, and a 6 hour bounded live window. The run opened WS, and after about 75s the service was active with 74 decisions, 74 movement commands, no runner error, and self at about `(21069,83333)` with HP 100.
...
Future validations are revalidation-only and should remain staged when run again.
