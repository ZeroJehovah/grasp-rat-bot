...
As of the later `bootstrap-0.4.357` source-builder extraction, the main file (`grasp-rat-bot.js`) is **320 lines** and keeps the Node/CDP CLI, status/diagnose flow, `--print-source` wrapper, and self-test delegation. The browser runtime source generator lives behind `src/browser/runtime-source.js` / `src/browser/bot-source.js`, with large browser runtime fragments split into dedicated source factories such as combat, opportunity, post-attack, opportunity-action, opportunity-route, opportunity-candidate, opportunity-choice, opportunity-pick, patrol, opportunity-clear, exit-motion, persistent-last-self, persistent-exit, persistent-clear, pending-exit-persistence, refresh-exit-detail, restored-coin-failures, login-snapshot-gate, runtime-diagnostics, tick-safety, attack-worth, array-count, choose-action, tick, startup, and coin-progress modules. The generated remote bot remains a single browser script.
