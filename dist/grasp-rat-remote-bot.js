...
(() => {
		  const baseConfig = {"dryRun":false,"once":false,"statusEvery":30000,"version":"bootstrap-0.4.266"};
		  const runtimeConfig = (() => {
		    try {
...
function hpDisplay(value) {
  ...
  function postLoginZoomFitDecision(measure) {
    ...
    return { done: true, direction: '', reason: 'fit' };
  }
  ...
  function postLoginZoomStepImproved(before, after, direction) {
    ...
    return String(direction || 'out') === 'in'
      ? afterRatio >= beforeRatio + minimumChange
      : afterRatio <= beforeRatio - minimumChange;
  }
