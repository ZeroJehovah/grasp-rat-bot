    ...
    try {
      ...
      statusServerChatTest = {
        ok: Boolean(
          ...
          && pageHtml.includes('function drawMapTrails(context, scene, markers, frame, progress = 1, retractProgress = progress)')
          && pageHtml.includes('function renderedMapTrailRetractGeometry(scene, camera, progress, retracting = mapTrailRetractingTails)')
          && pageHtml.includes('const paths = mapTrailRetractPaths(projected, progress);')
          && !pageHtml.includes('mapTrailFadeAlpha')
