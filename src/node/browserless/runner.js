    ...
    try {
      ...
      statusServerChatTest = {
        ok: Boolean(
          ...
          && pageHtml.includes('const opacity = mapTrailOpacity(pathSamples.at(-1).at, scene.trailNowMs, MAP_TRAIL_MAX_AGE_MS)')
          && pageHtml.includes('const MAP_LINE_ANIMATION_CYCLE_MS = 1800')
          && pageHtml.includes('const MAP_LINE_PULSE_PERIOD_MS = 1500')
          && pageHtml.includes('function traceMapSmoothPath(context, points)')
          && pageHtml.includes('context.quadraticCurveTo(')
          && pageHtml.includes('function startMapLineAnimation(scene, markers, animate = true)')
          ...
          && pageHtml.includes('context.lineWidth = dashed ? 1 : 1.1')
          && pageHtml.includes("targetRole === 'remote-snapshot'")
          && pageHtml.includes('context.setLineDash([6, 5])')
          && pageHtml.includes("color: remoteSnapshot ? '#9ca3af'")
          && pageHtml.includes('mapRemoteTargetPosition')
          && pageHtml.includes('context.setLineDash([7, 13])')
