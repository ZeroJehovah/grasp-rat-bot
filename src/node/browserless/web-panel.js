...
function lastExitPanelVisibleCore(status = {}) {
  const { realtimeOnline } = panelSessionFlagsCore(status);
...
<body class="panel-collapse-initializing">
  ...
  <script>
    ...
    function updateLastExitPanel(status) {
      ...
      if (!panel) return;
      const offline = status.stats?.offline || {};
      // A transport-recovery wait deliberately preserves the active stats
      // session, but the page is still offline from the realtime-control
      // perspective. Keep the latest actual exit visible during that gap.
