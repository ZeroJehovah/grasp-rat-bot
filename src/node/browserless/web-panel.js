...
function targetAuthoritySourceTextCore(source, target = null) {
  if (target?.cachedNavigationOnly === true) return 'WS最后位置';
...
function panelTargetRolesCore(status = {}) {
  ...
  // Recovery/wait can carry a retained profit mission for later arbitration,
  // but that memory is not a current panel target. Keep an explicit action
  // target so bounded profit holds still render their real objective.
  ...
  let primary = suppressRetainedTargets
    ? null
    : (explicit.primary || status?.combat?.primaryTarget || status?.profit?.mission?.navigationTarget || null);
  let secondary = suppressRetainedTargets
    ? null
    : (explicit.secondary || status?.combat?.secondaryTarget || null);
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .chat-row:last-child{border-bottom:0}.chat-time{color:var(--muted);font-size:11px;font-variant-numeric:tabular-nums;transform:translateY(2px)}.chat-author{min-width:0;color:var(--blue);font-weight:650;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.chat-text{min-width:0;overflow-wrap:anywhere;white-space:pre-wrap}
...
<body class="panel-collapse-initializing">
  ...
  <script>
    ...
    const sourceIpDeferredDetailText = status => dailyFirstLoginExempt(status)
      ? '已保留 3 个 IP，等待下一个登录时点；每日零点首次登录豁免仍有效，00:00:00 将直接使用主 IP 登录'
    ...
    const reasonMap = {
      ...
      'ignore-stale-coin-no-progress': '金币目标已失效，正在脱离并重新选路',
      'ignore-close-stale-coin': '近处金币目标已失效，正在脱离并重新选路',
      'ignore-near-stale-coin': '附近金币目标已失效，正在脱离并重新选路',
      'leave-stale-coin': '正在脱离已失效的金币目标',
      ...
      'daily-first-login-delay': '每日首次登录将在 00:00:00 准时开始',
