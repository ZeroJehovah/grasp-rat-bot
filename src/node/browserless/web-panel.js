...
<body>
  ...
  <script>
    ...
    async function refresh() {
      ...
      rows('runner', [['mode', s.runner?.mode], ['control', s.runner?.controlMode], ['profile', s.runner?.canaryProfile], ['running', s.runner?.running], ['readOnly', s.runner?.readOnly], ['dryRun', s.runner?.dryRun], ['combat', s.runner?.combatEnabled], ['action', s.runner?.currentAction?.kind || s.runner?.currentAction], ['error', s.runner?.lastError]]);
