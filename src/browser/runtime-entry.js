...
const __graspRatRuntimeStartup = (() => {
  ...
  const watchdogRuntime = require('./runtime/watchdog-heartbeat-runtime').createWatchdogHeartbeatRuntime({ bot, cfg, storage: localStorage, pageGlobal, documentRef: document, locationRef: location, fetchFn: fetch, now: Date.now, getSelf: () => getSelf(), summarizeSelf: (...args) => summarizeSelf(...args), getCurrentUserId: () => getCurrentUserId(), getSessionToken: () => getSessionToken(), summarizeControl: (...args) => summarizeControl(...args), combatTickActiveFromState: (...args) => combatTickActiveFromState(...args), recordRuntimeDiagnostics: detail => recordRuntimeDiagnosticsCore(bot, detail) });
  configureWatchdogApi = watchdogRuntime.configureWatchdog; summarizeWatchdogStatusApi = watchdogRuntime.summarizeWatchdogStatus; startWatchdogHeartbeatApi = watchdogRuntime.startWatchdogHeartbeat; startWatchdogHeartbeatApi();
