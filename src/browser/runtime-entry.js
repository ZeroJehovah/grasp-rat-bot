...
const __graspRatRuntimeStartup = (() => {
		  const { createRuntimeBootstrapBindings } = require('./runtime/runtime-bootstrap-bindings');
		  const runtimeBootstrapBindings = createRuntimeBootstrapBindings(__GRASP_RAT_RUNTIME_CONFIG__);
		  const {
		    pageGlobalObject,
		    resolvePageGlobal,
		    readPageGlobal,
...
const { createRuntimeStateBindings } = require('./runtime/runtime-state-bindings');
  ...
  const { postExitDecisionWithoutTargetCore: postExitDecisionWithoutTargetForStatusCore } = require('./runtime/exit-motion');
  const { readEnemyLeaveStreakBoundCore } = require('./runtime/exit-relogin');
  const { pendingExitRetryMsCore: pendingExitRetryMsForBotObjectCore, summarizePendingExitCore: summarizePendingExitForBotObjectCore } = require('./runtime/pending-exit');
      ...
      sequence: Number(preserved.combatLogging?.sequence || 0)
	    ...
	    importantLogging: {
	      ...
	      lastRemoteError: String(preserved.importantLogging?.lastRemoteError || '')
		    ...
		    loginSnapshotGate: normalizeLoginSnapshotGateStateCore(preserved.loginSnapshotGate, loginSnapshotSuccessRequiredCore()),
...
const { combatLogExitSummaryFromDecision } = require('./runtime/exit-summary');
      ...
      const { clearOfflineReloginHoldBoundCore: clearOfflineReloginHoldForCombatLogBoundCore, enemyReloginHoldRemainingMsBoundCore: enemyReloginHoldRemainingMsForCombatLogBoundCore, offlineReloginHoldRemainingMsBoundCore: offlineReloginHoldRemainingMsForCombatLogBoundCore } = require('./runtime/exit-relogin');
      const { pendingExitRetryMsCore: pendingExitRetryMsForCombatLogCore, summarizePendingExitCore: summarizePendingExitForCombatLogCore } = require('./runtime/pending-exit');
  ...
  const { pendingExitRetryMsCore: pendingExitRetryMsForControlLoginCore, summarizePendingExitCore: summarizePendingExitForControlLoginCore } = require('./runtime/pending-exit');
  ...
  const { pendingExitRetryMsCore: pendingExitRetryMsForLeaveCommandCore, summarizePendingExitCore: summarizePendingExitForLeaveCommandCore } = require('./runtime/pending-exit');
  const { pendingExitDisplayReasonCore: pendingExitDisplayReasonForLeaveCommandCore, leaveDetailHasHttp403Core: leaveDetailHasHttp403ForLeaveCommandCore, leaveDetailSucceededCore: leaveDetailSucceededForLeaveCommandCore } = require('./runtime/pending-exit');
  const { leaveCommandFailureMessageCore, summarizeLeaveCommandResultCore, leaveDetailFailedForClashRescueCore, clashLeaveRescueAttemptsCore, nextClashLeaveRescueStageCore, summarizeClashLeaveRescueResultCore, clashLeaveRescueRetryDetailCore, resetClashLeaveRescueRoundCore } = require('./runtime/leave-command');
