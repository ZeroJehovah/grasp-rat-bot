...
function createTargetOverlayRuntime(runtime = {}) {
  ...
  const firstFiniteNumber = typeof runtime.firstFiniteNumber === 'function'
    ? runtime.firstFiniteNumber
    : (...values) => values.map(Number).find(Number.isFinite);
  const dist = typeof runtime.dist === 'function'
    ? runtime.dist
    : (a, b) => Math.hypot(Number(a?.x) - Number(b?.x), Number(a?.y) - Number(b?.y));
  const isAlive = typeof runtime.isAlive === 'function'
    ? runtime.isAlive
    : e => e && e.life !== 'Dead' && e.life !== 'WaitingRevive' && !e.waiting_revive;
  const loginPointSafetyStatus = typeof runtime.loginPointSafetyStatus === 'function'
    ? runtime.loginPointSafetyStatus
    : () => null;
  const exitMotionStopLockRemainingMs = typeof runtime.exitMotionStopLockRemainingMs === 'function'
    ? runtime.exitMotionStopLockRemainingMs
    : () => 0;
  ...
  function renderTargetOverlay(decision = bot?.lastDecision) {
    try {
      ...
      if (!style || !target || !self) {
        ...
        const ctx = view?.overlay?.getContext('2d') || documentRef.getElementById(targetOverlayId)?.getContext('2d') || null;
        if (ctx) {
