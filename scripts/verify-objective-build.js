...
function main() {
  ...
  for (const file of BOOTSTRAP_FILES) {
    ...
    check(`${file} uses compact dot panel controls`, () => {
      ...
      assert(loginBody.includes('等待安全'), 'inline login button does not show safety wait state');
      ...
      assert(fallbackLoginBody.includes('rememberLoginGateBlock(loginGateBlock, reason)'), 'bootstrap fallback login does not record local login gate blocks');
      ...
      assert(forceBody.includes("result?.login?.reason === 'snapshot-gate'"), 'bootstrap force login does not preserve holds on snapshot/login-point gate blocks');
      assert(forceBody.includes('bootstrapLoginPointSafetyBlock(getBotStatus())'), 'bootstrap force fallback does not check login-point safety before clearing holds');
