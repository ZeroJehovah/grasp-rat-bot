...
function assertCombatMovementRuntimeSmoke() {
  ...
  const runtime = createCombatMovementRuntime({
    ...
    cfg: {
      ...
      combatStationarySpeed: 0.1,
      combatAimMovingScaleThreshold: 0.15,
      combatStrafeLockMs: 1000
    },
    speed: entity => entity === target ? 1 : 0,
    hpValue: entity => entity?.user_id === 'self' ? 100 : 90,
    dropValue: entity => Number(entity?.drop || 0),
    isCurrentlyActive: entity => Boolean(entity?.active),
...
async function main() {
  ...
  check('control login gate runtimes own extracted login safety bodies', () => {
    ...
    assert(runtimePostLoginZoomSource.includes("reason: 'view-radius-cap'"), 'post-login zoom does not cap outward zoom at the configured view radius');
    ...
    assert(runtimeDefaultsSource.includes('postLoginZoomMaxViewRadiusCm: 50000'), 'post-login zoom max view radius does not cap at 500m');
    ...
    assert(runtimeDefaultsSource.includes('postLoginZoomFitMaxSteps: 8'), 'post-login zoom measured step cap is not conservative');
    assert(runtimeDefaultsSource.includes('postLoginZoomFitMaxOutSteps: 4'), 'post-login zoom outward step cap is missing');
  ...
  check('combat runtime modules own target movement aim fire state and action bodies', () => {
    ...
    assert(/createCombatMovementRuntime\(\{[\s\S]*?\bisInvulnerable,/.test(runtimeCombatSource), 'combat runtime does not pass isInvulnerable into movement runtime');
