...
async function main() {
  ...
  const bundlerSpikeBuildSource = readText('scripts/build-runtime-helper-entry.js');
  const remoteBundledBuildSource = readText('scripts/build-remote-bot-bundled.js');
  const bundlerSpikeEntrySource = readText('src/browser/runtime-helper-entry.mjs');
  ...
  check('source modules split browser source generation while generated runtime stays single file', () => {
    ...
    assert(!fs.existsSync(path.join(ROOT, 'src/browser/runtime-fragments-source.js')), 'obsolete runtime fragments materializer file still exists');
    assert(runtimeSourceModule.includes("const { browserRuntimeFragmentEntries } = require('./runtime-fragment-registry')"), 'runtime source boundary does not own the runtime fragment registry dependency');
    assert(!runtimeSourceModule.includes("require('./runtime-fragments-source')"), 'runtime source still imports obsolete materializer file');
    assert(runtimeSourceModule.includes('function browserRuntimeBodySource(options = {})'), 'runtime body source adapter not found');
    assert(runtimeSourceModule.includes('return renderRuntimeFragments(browserRuntimeFragments(browserRuntimeConfig(options)));'), 'runtime body source boundary does not render the runtime fragment registry');
    assert(runtimeSourceModule.includes('function wrapBrowserRuntimeIife(source)'), 'runtime source boundary does not own the runtime IIFE wrapper');
    assert(runtimeSourceModule.includes('return wrapBrowserRuntimeIife(browserRuntimeBodySource(options));'), 'runtime source boundary does not wrap the rendered runtime body');
    ...
    assert(!runtimeSourceModule.includes("require('./runtime-bootstrap-source')"), 'runtime source materializer still owns provider imports');
    assert(runtimeSourceModule.includes('function materializeRuntimeFragments(entries)'), 'runtime fragment materializer not found');
    ...
    assert(runtimeSourceModule.includes('function browserRuntimeFragments(config)'), 'runtime fragments source factory not found');
    ...
    const runtimeBodySourceBody = functionBody(runtimeSourceModule, 'browserRuntimeBodySource');
    const runtimeIifeWrapperBody = functionBody(runtimeSourceModule, 'wrapBrowserRuntimeIife');
    const fragmentRegistryBody = functionBody(runtimeSourceModule, 'browserRuntimeFragments');
    const fragmentEntriesBody = functionBody(runtimeFragmentRegistryModule, 'browserRuntimeFragmentEntries');
    const fragmentMaterializerBody = functionBody(runtimeSourceModule, 'materializeRuntimeFragments');
    assert(runtimeSourceBody.includes('return wrapBrowserRuntimeIife(browserRuntimeBodySource(options));'), 'runtime source does not wrap the runtime body');
    ...
    assert(runtimeSourceModule.includes('function runtimeFragment(name, source)'), 'runtime fragment metadata helper not found');
    assert(runtimeSourceModule.includes("typeof name !== 'string'"), 'runtime fragment helper does not validate explicit names');
    assert(runtimeSourceModule.includes('source === undefined || source === null'), 'runtime fragment helper does not validate sources');
    assert(!runtimeSourceModule.includes('function runtimeFragmentName('), 'runtime fragment names should be explicit, not inferred');
  ...
  check('bundler scripts are wired without coupling spike and production entrypoints', () => {
    ...
    assert(rootPackage.scripts?.['build:runtime-helper-entry'] === 'node scripts/build-runtime-helper-entry.js', 'bundler spike build script not found');
    assert(rootPackage.scripts?.['test:runtime-helper-entry'] === 'node scripts/build-runtime-helper-entry.js --self-test', 'bundler spike self-test script not found');
    ...
    assert(!buildRemoteSource.includes('build-runtime-helper-entry') && !buildRemoteSource.includes('build-remote-bot-bundled'), 'production remote build is coupled to helper/candidate scripts instead of the shared bundler');
  ...
  check('bundler spike bundles shared and strategy helpers into a browser IIFE', () => {
    assert(bundlerSpikeEntrySource.includes("import runtimeUtils from './runtime/runtime-utils.js'"), 'bundler spike does not import runtime utils through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import displayFormat from './runtime/display-format.js'"), 'bundler spike does not import display helpers through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import targetWhitelist from './runtime/target-whitelist.js'"), 'bundler spike does not import target whitelist helpers through the browser runtime helper module');
    ...
    assert(bundlerSpikeEntrySource.includes("import coinDiagnostics from './runtime/coin-diagnostics.js'"), 'bundler spike does not import coin diagnostics through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import coinMotion from './runtime/coin-motion.js'"), 'bundler spike does not import coin motion through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import coinTarget from './runtime/coin-target.js'"), 'bundler spike does not import coin target through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import opportunityClear from './runtime/opportunity-clear.js'"), 'bundler spike does not import opportunity clear through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import postAttackDrop from './runtime/post-attack-drop.js'"), 'bundler spike does not import post-attack drop through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import dropMatchedKill from './runtime/drop-matched-kill.js'"), 'bundler spike does not import drop-matched kill through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import staminaBudget from './runtime/stamina-budget.js'"), 'bundler spike does not import stamina budget through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import opportunityConstants from './runtime/opportunity-constants.js'"), 'bundler spike does not import opportunity constants through the browser runtime helper module');
    assert(bundlerSpikeEntrySource.includes("import pageAdapter from './page-global-core.js'"), 'bundler spike does not import the shared page-global adapter');
    assert(bundlerSpikeEntrySource.includes("import arrayCountRuntime from './runtime/array-count.js'"), 'bundler spike does not import the browser runtime helper module');
    ...
    assert(bundlerSpikeEntrySource.includes("const HELPER_ENTRY_KEY = '__graspRatRuntimeHelperEntry'"), 'bundler spike global key not found');
    assert(bundlerSpikeEntrySource.includes("const CONFIG_KEY = '__GRASP_RAT_RUNTIME_HELPER_ENTRY_CONFIG__'"), 'bundler spike config key not found');
    assert(bundlerSpikeEntrySource.includes('pageAdapter.installPageGlobal(HELPER_ENTRY_KEY, installed);'), 'bundler spike does not install through the page-global adapter');
    ...
    assert(bundlerSpikeEntrySource.includes("pageAdapter.readPageLocalStorageJson('graspRatRuntimeHelperEntryProbe'"), 'bundler spike does not exercise localStorage adapter behavior');
    ...
    assert(bundlerSpikeEntrySource.includes('installRuntimeHelperEntry(runtimeConfig);'), 'bundler spike entry does not install itself');
    ...
    assert(bundlerSpikeBuildSource.includes("globalName: '__graspRatRuntimeHelperEntryBundle'"), 'bundler spike IIFE globalName not found');
    ...
    assert(bundlerSpikeBuildSource.includes("assert(source.includes('__graspRatRuntimeHelperEntry')"), 'bundler spike self-test does not assert the global key positively');
    ...
    assert(bundlerSpikeEntrySource.includes("import pendingExit from './runtime/pending-exit.js'"), 'bundler spike entry does not import pending-exit runtime adapter');
  ...
  check('coin progress failure helpers use strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/coin-progress.js'"), 'bundler spike does not import coin progress runtime adapter');
  ...
  check('coin route planner uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/coin-route.js'"), 'bundler spike does not import coin route runtime adapter');
  ...
  check('opportunity choice stability uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/opportunity-choice.js'"), 'bundler spike does not import opportunity choice runtime adapter');
  ...
  check('opportunity pick uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/opportunity-pick.js'"), 'bundler spike does not import opportunity pick runtime adapter');
  ...
  check('patrol direction uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/patrol.js'"), 'bundler spike does not import patrol runtime adapter');
  ...
  check('attack worth uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/attack-worth.js'"), 'bundler spike does not import attack-worth runtime adapter');
  ...
  check('exit motion uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/exit-motion.js'"), 'bundler spike does not import exit-motion runtime adapter');
  ...
  check('opportunity clear uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/opportunity-clear.js'"), 'bundler spike does not import opportunity clear runtime adapter');
  ...
  check('opportunity candidate construction uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/opportunity-candidates.js'"), 'bundler spike does not import opportunity candidates runtime adapter');
  ...
  check('post-attack drop wait uses strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/post-attack-drop.js'"), 'bundler spike does not import post-attack drop runtime adapter');
  ...
  check('stamina budget helpers use strategy module core', () => {
    ...
    assert(bundlerSpikeEntrySource.includes("from './runtime/stamina-budget.js'"), 'bundler spike does not import stamina-budget runtime adapter');
