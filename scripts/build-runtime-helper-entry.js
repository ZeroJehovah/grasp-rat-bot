...
async function selfTest() {
  ...
  assert(source.includes('function primePendingUnsafeExitLoginSuppressBoundCore'), 'exit-relogin pending unsafe suppress bound helper was not bundled');
  ...
  assert(status.dropMatchedKillVictim === 'Post Target', 'spike did not execute drop-matched kill helper');
  assert(status.dropMatchedKillStaminaMs === 150, 'spike did not preserve drop-matched kill stamina window');
  ...
  assert(status.pendingExitCoreRetryMs === 4500, 'spike did not execute pending-exit retry core');
  ...
  assert(status.exitReloginHealthyHighValueInjurySuppressed === true, 'spike did not suppress healthy high-value coin injury leave');
  assert(status.exitReloginLowHpHighValueInjurySuppressed === false, 'spike suppressed low-HP high-value coin injury leave');
  ...
  assert(status.exitReloginSuppressReuseUntil === 9000, 'spike did not reuse existing exit relogin suppress hold');
  assert(status.exitReloginSuppressReuseBotUntil === 9000, 'spike did not restore reused enemy hold onto bot state');
  assert(status.exitReloginSuppressReuseHold === 8000, 'spike did not preserve reused hold remaining time');
  assert(status.exitReloginSuppressReuseReason === 'combat leave', 'spike did not preserve reused suppress reason');
  assert(status.exitReloginSuppressReusePersisted === true, 'spike did not persist reused suppress detail');
  assert(status.exitReloginSuppressZeroUntil === 0, 'spike did not execute zero-delay suppress branch');
  assert(status.exitReloginSuppressZeroBotUntil === 0, 'spike did not clear zero-delay offline hold');
  assert(status.exitReloginSuppressZeroSkipped === true, 'spike did not mark zero-delay defensive skip');
  assert(status.exitReloginSuppressZeroPersisted === true, 'spike did not persist zero-delay suppress detail');
  assert(status.exitReloginSuppressNewUntil === 7000, 'spike did not create new suppress hold');
  assert(status.exitReloginSuppressNewDelay === 6000, 'spike did not honor suppress minimum delay');
